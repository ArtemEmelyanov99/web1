...
async function select_draw(id) {
	...
	s2.innerHTML = `<input type="checkbox" class="form-check-input" id="social" onclick="for_social(${id})" value = "0" />
				<label class="form-check-label" for="exampleCheck1">Социальные скидки</label>`;
	console.log(content);
...
// установка значения checkbox при выборе "Социальная скидка"
function for_social(id) {
...
function create_window(id) {
	...
	var social_check = document.getElementById("social"); // получение информации о checkbox для дальнейшей проверки при заполнении
	...
	if (social_check.value == 1) {
		dop_s.innerHTML = `
                        <p>Социальные скидки:</p>
                        <p> ${(
													Number(amount) -
													Number(amount) * content["socialDiscount"]*0,01
												).toFixed(1)}.</p>
                      `;
	}
